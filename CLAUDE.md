...
The opening hours (Mon–Fri 15–19, Sat 10–15) sit in two spots that must always agree: the footer and `openingHoursSpecification` in the JSON-LD. The phone number (+598 92 992 396) sits in eight: six `wa.me` links, `telephone` in the schema, and the visible footer text. Change one, change them all — a mismatch between the page and the Google Business Profile cancels out the local SEO signal it was meant to send.
